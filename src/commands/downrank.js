...
export default {
 data:new SlashCommandBuilder().setName('downrank').setDescription('Setzt den gewünschten Rang aus der gemeinsamen Framework-Rangstruktur.')
 ...
 async execute(i){
  ...
  const saveRank=db.transaction(()=>{
    ...
    db.prepare(`INSERT INTO personnel_actions(discord_id,action,area,old_rank,new_rank,performed_by,reason,created_at)
      VALUES(?,?,?,?,?,?,?,?)`).run(u.id,'DOWNRANK',applied.area,old,fw.name,i.user.id,reason,now);
  });
  saveRank();

  await i.editReply({embeds:[new EmbedBuilder().setTitle('⬇️ Degradierung').setDescription(`<@${u.id}> wurde auf den ausgewählten Framework-Rang gesetzt.`).addFields(
